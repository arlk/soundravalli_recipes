document.addEventListener("DOMContentLoaded", () => {
  ...
  async function initRecipeList() {
    ...
    try {
      ...
      recipeList.innerHTML =
        '<p class="no-results">Error loading recipes. Please ensure you are running on a web server.</p>';
      return;
    }
    ...
    searchInput.addEventListener("input", (e) => {
      ...
      const filteredRecipes = recipesIndex.filter(
        (recipe) =>
          recipe.title_en.toLowerCase().includes(searchTerm) ||
          (recipe.title_ta &&
            recipe.title_ta.toLowerCase().includes(searchTerm)) ||
  ...
  async function initRecipeDetail() {
    ...
    document.getElementById("recipe-subtitle").innerText =
      recipe.title_ta || "";
    ...
    function scaleIngredients(servings) {
      ...
      function renderMethod(methodList, header = "### Method") {
        ...
        const flattenedSteps = methodList.flatMap((step) =>
          typeof step === "string" ? step.split("\n") : [],
        );
      ...
      if (
        recipe.variations &&
        Array.isArray(recipe.variations) &&
